...
use web_sys::{CanvasRenderingContext2d as CTX, WebGlProgram, WebGlRenderingContext as GL};

use crate::aux_structs::GLMode;
use crate::aux_structs::{NormalizedPointObject, PointObject};
use crate::aux_structs::{
    CANVAS_DRAWN_OBJECT_SELECTED_DENOTATION_COLOR,
    CANVAS_DRAWN_OBJECT_UNDER_CURSOR_DENOTATION_COLOR, DRAWN_OBJECT_SELECTED_COLOR,
    DRAWN_OBJECT_TO_CANVAS_HEIGHT_SCALE, DRAWN_OBJECT_TO_CANVAS_WIDTH_SCALE,
    DRAWN_OBJECT_UNDER_CURSOR_COLOR, HINT_SHIFT_X, PAN_HINT_SHIFT_Y, ROTATION_HINT_SHIFT_Y,
    ZOOM_HINT_SHIFT_Y,
};
...
fn find_object_min_max_coordinates(
...
pub fn normalize_point_objects(
    uid: &mut u32,
    point_objects: &Vec<PointObject>,
    canvas_width: f32,
    canvas_height: f32,
) -> Vec<NormalizedPointObject> {
    let aspect = canvas_width / canvas_height;
    let mut normalized_point_objects = Vec::new();

    let (x_min, x_max, y_min, y_max, z_min, z_max) = find_object_min_max_coordinates(point_objects);
    ...
    for point_object in point_objects.iter() {
        *uid += 1;
        ...
        let mut x = (point_object.x * multiplier - (x_max + x_min) * multiplier / 2.0)
            / (min_canvas_side / 2.0)
            * min_drawn_object_to_canvas_scale;
        if x.is_nan() {
            x = 0.0;
        }
        let mut y = (point_object.y * multiplier - (y_max + y_min) * multiplier / 2.0)
            / (min_canvas_side / 2.0)
            * min_drawn_object_to_canvas_scale;
        if y.is_nan() {
            y = 0.0;
        }
        let mut z = (point_object.z * multiplier - (z_max + z_min) * multiplier / 2.0)
            / (min_canvas_side / 2.0)
            * min_drawn_object_to_canvas_scale;
        if z.is_nan() {
            z = 0.0;
...
pub fn add_denotation(
    ...
    ctx.fill_text(denotation, pixel_x as f64, pixel_y as f64)
        .unwrap();
}
...
pub fn define_drawn_object_color(
    gl_mode: &GLMode,
    uid: u32,
    selected_color: &[u8; 4],
...
pub fn define_drawn_object_denotation_color(
    uid: u32,
    selected_color: &[u8; 4],
    under_cursor_color: &[u8; 4],
    initial_denotation_color: &str,
) -> String {
    let transformed_uid = transform_u32_to_array_of_u8(uid);
    if transformed_uid == *selected_color {
...
pub fn add_hints(ctx: &CTX, canvas_width: f32, canvas_height: f32) {
    ...
    ctx.fill_text(rotate_hint, hint_x as f64, rotate_hint_y as f64)
        .unwrap();
    let zoom_hint_y = canvas_height * ZOOM_HINT_SHIFT_Y;
    let zoom_hint = "Zoom - Mouse Wheel";
    ctx.fill_text(zoom_hint, hint_x as f64, zoom_hint_y as f64)
        .unwrap();
    let pan_hint_y = canvas_height * PAN_HINT_SHIFT_Y;
    let pan_hint = "Pan - Shift + Left Mouse Button";
    ctx.fill_text(pan_hint, hint_x as f64, pan_hint_y as f64)
        .unwrap();
}
