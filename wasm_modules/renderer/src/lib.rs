...
use web_sys::{
    CanvasRenderingContext2d as CTX, WebGlProgram, WebGlRenderingContext as GL, WebGlShader,
};
...
use aux_functions::{
    add_denotation, add_hints, define_drawn_object_denotation_color, dispatch_custom_event,
    initialize_shaders, normalize_point_objects, transform_u32_to_array_of_u8,
};
use aux_structs::{Buffers, DrawnObject, NormalizedPointObject, PointObject, ShadersVariables};
...
use aux_structs::{
    ...
    CANVAS_DRAWN_DISPLACEMENTS_DENOTATION_COLOR, CANVAS_DRAWN_ELEMENTS_DENOTATION_COLOR,
    CANVAS_DRAWN_FORCES_DENOTATION_COLOR, CANVAS_DRAWN_NODES_DENOTATION_COLOR,
    CANVAS_DRAWN_POINTS_DENOTATION_COLOR, CS_AXES_CAPS_BASE_POINTS_NUMBER, CS_AXES_CAPS_HEIGHT,
    CS_AXES_CAPS_WIDTH, CS_AXES_SCALE, CS_AXES_X_SHIFT, CS_AXES_Y_SHIFT, CS_AXES_Z_SHIFT,
    DRAWN_DISPLACEMENTS_CAPS_BASE_POINTS_NUMBER, DRAWN_DISPLACEMENTS_CAPS_HEIGHT,
    DRAWN_DISPLACEMENTS_CAPS_WIDTH, DRAWN_DISPLACEMENTS_DENOTATION_SHIFT_X,
    DRAWN_DISPLACEMENTS_DENOTATION_SHIFT_Y, DRAWN_ELEMENTS_DENOTATION_SHIFT,
    DRAWN_FORCES_CAPS_BASE_POINTS_NUMBER, DRAWN_FORCES_CAPS_HEIGHT, DRAWN_FORCES_CAPS_WIDTH,
    DRAWN_FORCES_DENOTATION_SHIFT_X, DRAWN_FORCES_DENOTATION_SHIFT_Y, DRAWN_FORCES_LINE_LENGTH,
    DRAWN_POINT_OBJECT_DENOTATION_SHIFT, HINTS_COLOR,
};
...
impl Renderer {
    ...
    fn update_normalized_point_objects(&mut self) {
        self.state.uid = 0;
        let normalized_point_objects = normalize_point_objects(
            &mut self.state.uid,
        ...
        log(&format!(
            "{:?}, {:?}",
            self.props.point_objects, self.state.normalized_point_objects
        ));
    }
    ...
    pub fn update_point_object(
        ...
        if let Some(position) = self.props.point_objects.iter().position(|point_object| {
            ...
            let error_message = format!(
                "Renderer: Update {} action: {} with number \
                {} does not exist!",
                point_object_type.as_str().to_lowercase(),
                point_object_type.as_str(),
                number
            );
    ...
    pub fn select_object(&mut self) -> Result<(), JsValue> {
        ...
        {
            let selected_point_object = &self.state.normalized_point_objects[position];
            match selected_point_object.object_type {
                PointObjectType::Point => {
                    ...
                    dispatch_custom_event(detail, CLIENT_MESSAGE, EVENTS_TARGET)?;
                }
    ...
    fn render(&mut self) -> Result<(), JsValue> {
        ...
        if !self.state.normalized_point_objects.is_empty() {
            ...
            drawn_object.add_point_object(
                &self.state.normalized_point_objects,
                GLMode::Selection,
            ...
            gl.uniform_matrix4fv_with_f32_array(
                Some(&shaders_variables.projection_matrix),
                false,
                &projection_matrix,
            );
            gl.uniform_matrix4fv_with_f32_array(
                Some(&shaders_variables.model_view_matrix),
                false,
                &model_view_matrix,
            );
        ...
        let mut model_view_matrix = mat4::new_identity();
        let mat_to_translate = model_view_matrix;
        let y_shift = if CS_AXES_Y_SHIFT > 0.0 {
            1.0 - (1.0 - CS_AXES_Y_SHIFT) * aspect
        } else {
            -1.0 + (1.0 + CS_AXES_Y_SHIFT) * aspect
        };
        mat4::translate(
        ...
        add_denotation(
            &ctx,
            &[
                1.0 + AXIS_X_DENOTATION_SHIFT_X,
                0.0 + AXIS_X_DENOTATION_SHIFT_Y,
                0.0,
                1.0,
            ],
            &model_view_matrix,
        ...
        add_denotation(
            &ctx,
            &[
                0.0 + AXIS_Y_DENOTATION_SHIFT_X,
                1.0 + AXIS_Y_DENOTATION_SHIFT_Y,
                0.0,
                1.0,
            ],
            &model_view_matrix,
        ...
        add_denotation(
            &ctx,
            &[
                0.0 + AXIS_Z_DENOTATION_SHIFT_X,
                0.0 + AXIS_Z_DENOTATION_SHIFT_Y,
                1.0 + AXIS_Z_DENOTATION_SHIFT_Z,
                1.0,
            ],
            &model_view_matrix,
            width as f32,
