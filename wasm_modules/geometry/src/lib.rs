...
impl Line {
    ...
    fn extract_number_and_points_numbers(&self) -> (u32, u32, u32) {
        (
            self.number,
            self.start_point.borrow().extract_number(),
...
impl Geometry {
    ...
    fn clear_deleted_lines_by_action_id(&mut self, action_id: &u32) {
        while let Some(position) = self
            .deleted_lines
            .iter()
            .position(|line| line.action_id_greater_or_same(&action_id))
        {
    ...
    fn clear_deleted_points_by_action_id(&mut self, action_id: &u32) {
        while let Some(position) = self
            .deleted_points
            .iter()
            .position(|point| point.borrow().action_id_greater_or_same(&action_id))
        {
    ...
    pub fn add_point(
            ...
            .is_some()
        {
            let error_message = &format!(
                "Geometry: Add point action: Point with \
            ...
            .is_some()
        {
            let error_message = &format!(
                "Geometry: Add point action: Point with \
                coordinates {}, {}, {} does already exist!",
                x, y, z
            );
            return Err(JsValue::from(error_message));
        }
        ...
        log(&format!(
            "Geometry: Points: {:?}, lines: {:?}, deleted points: {:?}, \
            deleted lines {:?}",
            self.points, self.lines, self.deleted_points, self.deleted_lines
        ));
        Ok(())
    }
    ...
    pub fn update_point(
        ...
        {
            let error_message = &format!(
                "Geometry: Update point action: Point with \
                coordinates {}, {}, {} does already exist!",
                x, y, z
            );
            return Err(JsValue::from(error_message));
            ...
            .position(|point| point.borrow().number_same(number))
        {
            ...
            let error_message = format!(
                "Geometry: Update point action: \
                The point with number {} could not be updated because it does not exist!",
                number
            );
    ...
    pub fn delete_point(
            ...
            .position(|point| point.borrow().number_same(number))
        {
            while let Some(position) = self
                .lines
                .iter()
                .position(|line| line.point_belongs(number))
            {
            ...
            let error_message = &format!(
                "Geometry: Delete point action: Point with \
    ...
    pub fn undo_delete_point(
        ...
        if let Some(position) = self.deleted_points.iter().position(|point| {
            ...
            while let Some(position) = self
                .deleted_lines
                .iter()
                .position(|line| line.action_id_same(&action_id))
            {
            ...
            let error_message = &format!(
                "Geometry: Undo delete point action: Point with \
                number {} does not exist!",
                number
            );
            return Err(JsValue::from(error_message));
    ...
    pub fn add_line(
        ...
        {
            let error_message = &format!(
                "Geometry: Add line action: Line with \
                number {} does already exist!",
                number
            );
            return Err(JsValue::from(error_message));
        ...
        if self
            .lines
            .iter()
            .position(|line| line.start_and_end_points_same(start_point_number, end_point_number))
            .is_some()
        {
            let error_message = &format!(
                "Geometry: Add line action: Line with \
                point number {} and {} does already exist!",
        ...
        let start_point = {
            if let Some(position) = self
                .points
                .iter()
                .position(|point| point.borrow().number_same(start_point_number))
            {
                Ok(Rc::clone(&self.points[position]))
            } else {
                let error_message = &format!(
                    "Geometry: Add line action: Point with \
                        number {} does not exist!",
                    start_point_number
                );
                Err(JsValue::from(error_message))
        ...
        let end_point = {
            if let Some(position) = self
                .points
                .iter()
                .position(|point| point.borrow().number_same(end_point_number))
            {
                Ok(Rc::clone(&self.points[position]))
            } else {
                let error_message = &format!(
    ...
    pub fn update_line(
        ...
        {
            let error_message = &format!(
                "Geometry: Update line action: Line with \
                point number {} and {} does already exist!",
                start_point_number, end_point_number
            );
            return Err(JsValue::from(error_message));
        ...
        let start_point = {
                ...
                .position(|point| point.borrow().number_same(start_point_number))
            {
                ...
                let error_message = &format!(
                    "Geometry: Update line action: Point with \
                        number {} does not exist!",
                    start_point_number
                );
                Err(JsValue::from(error_message))
        ...
        if let Some(position) = self.lines.iter().position(|line| line.number_same(number)) {
            ...
            log(&format!(
                "Geometry: Points: {:?}, lines: {:?}, deleted points: {:?}, deleted \
                lines {:?}",
                self.points, self.lines, self.deleted_points, self.deleted_lines
            ));
        } else {
    ...
    pub fn delete_line(
        ...
        if let Some(position) = self.lines.iter().position(|line| line.number_same(number)) {
            ...
            let error_message = &format!(
                "Geometry: Delete line action: Line with \
                number {} does not exist!",
                number
            );
            return Err(JsValue::from(error_message));
    ...
    pub fn undo_delete_line(
            ...
            .position(|line| line.action_id_same(&action_id) && line.number_same(number))
        {
            ...
            let error_message = &format!(
                "Geometry: Undo delete line action: Line with \
                number {} does not exist!",
                number
            );
            return Err(JsValue::from(error_message));
    ...
    pub fn show_point_info(&mut self, number: u32) -> Result<(), JsValue> {
            ...
            .position(|point| point.borrow().number_same(number))
        {
            ...
            log(&format!(
                "You selected point with number: {}, x: {}, y: {}, z: {}",
                number, x, y, z
            ));
        } else {
            let error_message = &format!(
                "Geometry: Show point info action: Point with \
                number {} does not exist!",
                number
            );
            return Err(JsValue::from(error_message));
    ...
    pub fn add_whole_geometry_to_preprocessor(
        &self,
        is_action_id_should_be_increased: bool,
    ) -> Result<(), JsValue> {
        for point in &self.points {
            let (point_number, x, y, z) = point.borrow().extract_number_and_coordinates();
