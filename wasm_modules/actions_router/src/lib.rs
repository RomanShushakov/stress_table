...
extern "C" {
    ...
    fn undo_delete_point_from_geometry(
        action_id: u32,
        number: u32,
        is_action_id_should_be_increased: bool,
    ) -> Result<(), JsValue>;
    ...
    fn undo_delete_line_from_geometry(
        action_id: u32,
        number: u32,
        is_action_id_should_be_increased: bool,
    ) -> Result<(), JsValue>;
...
impl ActionsRouter {
    ...
    fn handle_update_point_message(&mut self, point_data: &Value) -> Result<(), JsValue> {
        ...
        let old_x_value = point_data["old_point_values"]["x"]
            .to_string()
            .parse::<f64>()
            .or(Err(JsValue::from(
                "Actions router: Update point action: \
        ...
        let old_y_value = point_data["old_point_values"]["y"]
            .to_string()
            .parse::<f64>()
            .or(Err(JsValue::from(
                "Actions router: Update point action: \
        ...
        let old_z_value = point_data["old_point_values"]["z"]
            .to_string()
            .parse::<f64>()
            .or(Err(JsValue::from(
                "Actions router: Update point action: \
        ...
        let new_x_value = point_data["new_point_values"]["x"]
            .as_str()
            .ok_or(JsValue::from(
                "Actions router: Update point action: \
        ...
        let new_y_value = point_data["new_point_values"]["y"]
            .as_str()
            .ok_or(JsValue::from(
                "Actions router: Update point action: \
        ...
        let new_z_value = point_data["new_point_values"]["z"]
            .as_str()
            .ok_or(JsValue::from(
                "Actions router: Update point action: \
    ...
    fn handle_add_line_message(&mut self, line_data: &Value) -> Result<(), JsValue> {
        let action_id = line_data["actionId"]
            .to_string()
            .parse::<u32>()
            .or(Err(JsValue::from(
                "Actions router: Add point action: Action id could not be converted to u32!",
            )))?;
        let number = line_data["number"]
            .as_str()
            .ok_or(JsValue::from(
        ...
        let is_action_id_should_be_increased = true;
        let action_type = ActionType::AddLine(
            line_number,
            start_point_number,
            end_point_number,
            is_action_id_should_be_increased,
    ...
    fn handle_update_line_message(&mut self, line_data: &Value) -> Result<(), JsValue> {
        ...
        let old_start_point_number = line_data["old_line_values"]["start_point"]
            .to_string()
            .parse::<u32>()
            .or(Err(JsValue::from(
                "Actions router: Update line action: \
        ...
        let old_end_point_number = line_data["old_line_values"]["end_point"]
            .to_string()
            .parse::<u32>()
            .or(Err(JsValue::from(
                "Actions router: Update line action: \
        ...
        let new_start_point_number = line_data["new_line_values"]["start_point"]
            .as_str()
            .ok_or(JsValue::from(
                "Actions router: Update line action: \
        ...
        let new_end_point_number = line_data["new_line_values"]["end_point"]
            .as_str()
            .ok_or(JsValue::from(
                "Actions router: Update line action: \
        ...
        let is_action_id_should_be_increased = true;
        let action_type = ActionType::UpdateLine(
            line_number,
    ...
    fn handle_undo_message(&mut self, undo_data: &Value) -> Result<(), JsValue> {
        let action_id = undo_data["actionId"]
            .to_string()
            .parse::<u32>()
            .or(Err(JsValue::from(
                "Actions router: Redo action: \
                Action id could not be converted to u32!",
            )))?;
        if let Some(position) = self
            .active_actions
            .iter()
            .position(|action| action.action_id_same(action_id))
        {
            ...
            match &undo_action.action_type {
                ...
                ActionType::UpdatePoint(point_number, old_coordinates, new_coordinates, _) => {
                    let is_action_id_should_be_increased = false;
                    let action_type = ActionType::UpdatePoint(
                        point_number.clone(),
                        new_coordinates.clone(),
                        old_coordinates.clone(),
                        is_action_id_should_be_increased,
                    );
                    let action = Action::create(action_id, action_type);
                    let add_to_active_actions = false;
                ...
                ActionType::DeletePoint(point_number, _) => {
                    let is_action_id_should_be_increased = false;
                    let action_type = ActionType::UndoDeletePoint(
                        point_number.clone(),
                        is_action_id_should_be_increased,
                    );
                    let action = Action::create(action_id, action_type);
                ...
                ActionType::AddLine(line_number, _, _, _) => {
                    let is_action_id_should_be_increased = false;
                    let action_type = ActionType::DeleteLine(
                        line_number.clone(),
                        is_action_id_should_be_increased,
                    );
                    let action = Action::create(action_id, action_type);
                ...
                ActionType::UpdateLine(
                    ...
                    let action_type = ActionType::UpdateLine(
                        line_number.clone(),
                        new_start_point_number.clone(),
                        new_end_point_number.clone(),
                        old_start_point_number.clone(),
                        old_end_point_number.clone(),
                        is_action_id_should_be_increased,
                    );
                    let action = Action::create(action_id, action_type);
                    let add_to_active_actions = false;
                ...
                ActionType::DeleteLine(line_number, _) => {
                    let is_action_id_should_be_increased = false;
                    let action_type = ActionType::UndoDeleteLine(
                        line_number.clone(),
                        is_action_id_should_be_increased,
                    );
                    let action = Action::create(action_id, action_type);
    ...
    fn handle_redo_message(&mut self, redo_data: &Value) -> Result<(), JsValue> {
        let action_id = redo_data["actionId"]
            .to_string()
            .parse::<u32>()
            .or(Err(JsValue::from(
                "Actions router: Redo action: \
    ...
    fn handle_selected_point_number_message(
        &mut self,
        selected_point_number: &Value,
    ) -> Result<(), JsValue> {
        let point_number =
    ...
    fn handle_current_action(&mut self) -> Result<(), JsValue> {
        if let Some((action, add_to_active_actions)) = &self.current_action {
            ...
            match action_type {
                ...
                ActionType::DeletePoint(point_number, is_action_id_should_be_increased) => {
                    let number = point_number.get_number();
                    delete_point_from_geometry(
                        action_id,
                        number,
                        *is_action_id_should_be_increased,
                    )?;
                ...
                ActionType::UndoDeletePoint(point_number, is_action_id_should_be_increased) => {
                    let number = point_number.get_number();
                    undo_delete_point_from_geometry(
                        action_id,
                        number,
                        *is_action_id_should_be_increased,
                    )?;
                ...
                ActionType::DeleteLine(line_number, is_action_id_should_be_increased) => {
                    let number = line_number.get_number();
                    delete_line_from_geometry(
                        action_id,
                        number,
                        *is_action_id_should_be_increased,
                    )?;
                ...
                ActionType::UndoDeleteLine(line_number, is_action_id_should_be_increased) => {
                    let number = line_number.get_number();
                    undo_delete_line_from_geometry(
                        action_id,
                        number,
                        *is_action_id_should_be_increased,
                    )?;
    ...
    pub fn handle_message(&mut self, message: JsValue) -> Result<(), JsValue> {
        ...
        for action in &self.active_actions {
            ...
            let action_type = &action.action_type;
            log(&format!(
                "Actions router active actions: Action id: {:?}, action type: {:?}",
                action_id, action_type
            ));
        }
        log(&format!(
            "Actions router: The number of active actions: {}",
        ...
        for action in &self.undo_actions {
            ...
            let action_type = &action.action_type;
            log(&format!(
                "Actions router undo actions: Action id: {:?}, action type: {:?}",
                action_id, action_type
            ));
        }
        log(&format!(
            "Actions router: The number of undo actions: {}",
