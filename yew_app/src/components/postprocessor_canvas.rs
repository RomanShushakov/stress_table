        ...
        if !self.props.nodes.is_empty()
        {
            ...
            for (reaction, reaction_value) in &self.props.analysis_result.reactions
            {
                let node_position = drawn_nodes
                    .iter()
                    .position(|node| node.number == reaction.node_number).unwrap();
                let drawn_node = drawn_nodes[node_position].to_owned();
                context.set_fill_style(&CANVAS_REACTION_COLOR.into());

                match reaction.component
                {
                    AxisComponent::U =>
                        {
                            if *reaction_value > 0f64
                            {
                                context.begin_path();
                                context.move_to(drawn_node.x + axis_line_length / 25f64, drawn_node.y);
                                context.set_stroke_style(&CANVAS_REACTION_COLOR.into());
                                context.line_to(
                                ...
                                context.fill();
                                context.move_to(
                                    drawn_node.x + axis_line_length / 4f64 + axis_line_length / 30f64,
                                    drawn_node.y - axis_line_length / 30f64);
                                context.line_to(
                                    drawn_node.x + axis_line_length / 4f64 - axis_line_length / 30f64,
                                    drawn_node.y + axis_line_length / 30f64);
                            ...
                            if *reaction_value < 0f64
                            {
                                ...
                                context.fill_text(
                                    &format!("{:.2}", reaction_value),
                                    drawn_node.x - axis_line_length / 2f64 -
                                        (format!("{:.2}", reaction_value).chars().count() as f64 * axis_line_length / 14f64),
                                    drawn_node.y - axis_line_length / 14f64)
                                    .unwrap();
                                context.stroke();
                            }
                    ...
                    AxisComponent::V =>
                        {
                            if *reaction_value > 0f64
                            {
                                ...
                                context.move_to(
                                    drawn_node.x + axis_line_length / 30f64,
                                    drawn_node.y - axis_line_length / 4f64 - axis_line_length / 30f64);
                                context.line_to(
                                    drawn_node.x - axis_line_length / 30f64,
                                    drawn_node.y - axis_line_length / 4f64 + axis_line_length / 30f64);
                                context.set_font(&format!("{}px Serif", axis_line_length / 7f64));
                    ...
                    AxisComponent::ThetaW =>
                        {
                            if *reaction_value > 0f64
                            {
                                ...
                                context.fill_text(
                                    &format!("{:.2}", reaction_value),
                                    drawn_node.x - axis_line_length / 3f64 * f64::consts::FRAC_1_SQRT_2 -
                                        (format!("{:.2}", reaction_value).chars().count() as f64 + 1f64) * axis_line_length / 14f64,
                                    drawn_node.y - axis_line_length / 3f64 * f64::consts::FRAC_1_SQRT_2)
                                    .unwrap();
                                context.stroke();
                            }
