...
pub fn matrices_dimensions_conformity_check<'a, T, V>(lhs: &'a ExtendedMatrix<T, V>,
    ...
    where T: Copy +PartialEq + Mul<Output = T> + Add<Output = T> + Sub<Output = T> +
             Div<Output = T> + Rem<Output = T> + Default + From<FEUInt> +
             Into<FEUInt> + Eq + Hash + SubAssign + Debug + PartialOrd + 'static,
          V: Copy + Default + Mul<Output = V> + Div<Output = V> + Sub<Output = V> +
             Add<Output = V> + From<FEFloat> + Debug + PartialEq + AddAssign + MulAssign +
             SubAssign + Into<FEFloat> + 'static,
{
...
pub fn remove_zero_values<T, V>(indexes: &mut Vec<T>, values: &mut Vec<V>)
    where V: Copy + Default + PartialEq + Into<FEFloat>
{
